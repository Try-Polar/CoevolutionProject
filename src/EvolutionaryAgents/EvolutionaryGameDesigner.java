...
public class EvolutionaryGameDesigner {
	...
	public void singleSymbolRun()
	{
		...
		List<Symbol> a = popSymbols.get(0);
		List<Symbol> b = new LinkedList<Symbol>(popSymbols.get(0));
		System.out.println("~~~~~~~~INITIAL~~~~~~~~");
		System.out.println("--------GAME 0------------");
		gameDesigner.writeSymbolsToFile(a);
		System.out.println("--------GAME 1------------");
		gameDesigner.writeSymbolsToFile(b);
		...
		a = popSymbols.get(0);
		b = new LinkedList<Symbol>(popSymbols.get(0));
		System.out.println("~~~~~~~~POST ASSIGNMENT~~~~~~~~");
		System.out.println("--------GAME 0------------");
		gameDesigner.writeSymbolsToFile(a);
		System.out.println("--------GAME 1------------");
		gameDesigner.writeSymbolsToFile(b);
		
		crossovers = gameDesigner.onePointCrossover(popSymbols.get(0), popSymbols.get(1));
		popSymbols.set(0, crossovers.get(0));
		popSymbols.set(1, crossovers.get(1));
	...
	private List<List<Symbol>> crossoverMutation( List<List<Symbol>> popSymbols)
	{
		...
		for (int i=0; i < populationSize/2; i++)
		{
			...
			if (rnd.nextDouble() < crossoverProbability) {
				...
				popSymbols.set(b, crossovers.get(1)); //Java is probably using references for this so its probably not necessary
			}
		}
	...
	public double evaluationFunction(List<Symbol> individual)
	{
		//If game does not run return negative score for now if game runs return score of 0
				...
				try {
					...
					for (int i=0; i < runs; i++) {
						doNothingScore = ArcadeMachine.runOneGame(game, recordLevelFile, false, doNothingController, null, 5, 0);
						MCTSScore = ArcadeMachine.runOneGame(game, recordLevelFile, false, sampleMCTSController, null, 5, 0);
						MCTSAverageScore += MCTSScore[1];
						doNothingAverageScore += doNothingScore[1];
						if (MCTSScore[0] == 0) {
							MCTSWins++;
				...
				} else if (Math.min(MCTSWins, doNothingWins) != 0) {
					RDWins = ((MCTSWins - doNothingWins)/ Math.min(MCTSWins, doNothingWins));
				} else {
					RDWins = 0;
				}
